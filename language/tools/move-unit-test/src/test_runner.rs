...
fn unit_cost_table(number_of_native_functions: usize) -> CostTable {
    let mut cost_schedule = zero_cost_schedule(number_of_native_functions);
...
impl TestRunner {
    pub fn new(
        ...
        let native_function_table_len = native_function_table.len();
        Ok(Self {
            testing_config: SharedTestingConfig {
                ...
                cost_table: unit_cost_table(native_function_table_len),
                source_files,
                check_stackless_vm,
                verbose,
